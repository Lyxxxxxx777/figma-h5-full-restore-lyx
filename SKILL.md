...
**任务真正开始动手的那一刻记录开始时间**。定义：所有前置操作已完成、所有前置问题已问完之后 —— 即必传输入已收齐 ✅、环境检查通过（Vibma / Chrome DevTools MCP 已接上）✅、项目约定嗅探完成 ✅、「是否有需求文档」等问题已问完 ✅ —— **正式进入第 0 步激活底层 skill 之前**。

> ⚠️ 与用户来回确认参数、等待用户补齐 Figma 链接、等用户回答"是否有 PRD"的等待时间**不计入**任务耗时。

### 计时终点

任务完成（第 3 步自测通过 + 临时文件清理完成）时记录完成时间。

### Token 用量与费用（best-effort，能取则取，取不到严禁编造）

模型自身无法直接感知 token 消耗，但 **Codemaker 运行时的会话文件中有真实数据**：

`~/.codemaker/session-store/v1/<workspace-hash>/sessions/<sessionId>.json` 内每条 assistant 消息记录：

- `consumedTokensTotal` — 该条消息消耗 token（含 input / output / skill / rule / mcp）
- `price.totalCost` — 该条消息费用（平台计价口径）

**操作**：

1. 任务开始时（记录开始时间的同时）：在 `~/.codemaker/session-store/v1/*/sessions/` 下按 `*.meta.json` 的 `chatWorkspace` 匹配当前项目根、取最近更新的 `<sessionId>.json`，**累加全部消息的 `consumedTokensTotal` 与 `price.totalCost`** 作基线
2. 任务完成时：再次读取同一会话文件重新求和，**差值** = 本任务 token 消耗与费用
3. ⚠️ 两个字段是**逐条消息**的消耗（非会话累计值），必须求和；求和时过滤 `isOutdatedTokens: true` 的消息（已作废记录，累加会多算）
4. 数值是读取时刻的快照，日志写入有轻微延迟（进行中的最后一条可能未落盘），属正常误差
5. 非 Codemaker 运行时无此日志 → 输出 `token 用量：获取不到（运行时未暴露）`，**严禁估算 / 编造数值**
6. token 按任务级统计，不按页面拆分（消息明细虽带时间戳可聚合，但成本大于收益）

### 完成时必须输出

| 字段 | 说明 |
|---|---|
| 任务开始时间 | 真正动手那一刻的时间戳（YYYY-MM-DD HH:mm:ss） |
| 任务完成时间 | 自测通过的时间戳（YYYY-MM-DD HH:mm:ss） |
| 总耗时 | 完成时间 - 开始时间 |
| 完成的页面和完成度 | 每个页面名 + 完成度（如 100%；未达 100% 时注明原因） |
| Token 用量 | 本任务消耗 token（best-effort，取不到则注明"获取不到"） |
| 费用 | 本任务费用（best-effort，取不到则注明"获取不到"） |

### 多页面任务

需完成多个页面时，**每个页面单独记录开始 / 完成时间并计算耗时**，总结报告中列出每页耗时表：

```markdown
### 耗时统计
- 任务开始时间: 2026-09-02 17:00:00
- 任务完成时间: 2026-09-02 19:30:00
- 总耗时: 2h 30m
- Token 用量: 152,308 tokens（或：获取不到）
- 费用: $1.25（或：获取不到）
...
> ⏱️ **进入本步前记录任务开始时间，并读取 token 基线**（见 Task Timing 章节）。多页面任务从第一个页面开始逐页记录开始 / 完成时间。
...
- 总耗时: Xh Ym
- Token 用量: N tokens（best-effort；取不到则写"获取不到"）
- 费用: <金额>（best-effort；取不到则写"获取不到"）
- 完成页面与完成度:

| 页面 | 耗时 | 完成度 |
|---|---|---|
| <页面名> | <单页耗时> | <如 100% / 90% 及原因> |
